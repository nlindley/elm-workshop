            ...
            case result of
                ...
                Err error ->
                    let
                        errorMessage =
                            case error of
                                Http.BadPayload what _ ->
                                    "Unexpected response " ++ what

                                Http.BadUrl _ ->
                                    "You screwed up the URL"

                                Http.BadStatus response ->
                                    "Unexpected status " ++ (toString response.status.code) ++ " " ++ response.status.message

                                Http.Timeout ->
                                    "Things seem to be taking too long"

                                Http.NetworkError ->
                                    "Something is up with the network"
                    in
